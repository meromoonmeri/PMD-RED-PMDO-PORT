...
from pmdo_ground_writer import write_ground_cells
from cell_cycle_renderer import render_cell_cycles
...
def entities(src):
 ...
 return marks,sp
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--pret',default='/tmp/pmd-red');ap.add_argument('--out',required=True);ap.add_argument('--map-id',action='append');ap.add_argument('--max-period',type=int,default=None,help='Deprecated: cycles are now cell-local; retained for CLI compatibility.');ap.add_argument('--limit',type=int);a=ap.parse_args();pret=Path(a.pret);data=pret/'data/map_bg';deps=json.load(open(ROOT/'map_dependencies.json'));keys=a.map_id or list(deps);done=[];blocked=[]
 for key in keys:
  if a.limit and len(done)>=a.limit:break
  dep=deps[key]
  if dep['bpl'].upper() in DUNGEON_BACKED:blocked.append((key,'dungeon_blobs'));continue
  root=Path(a.out);asset=dep['bpl'].lower();gp=root/'Grounds'/f'{asset}.rsground'
  if gp.exists() and (root/'Manifests'/(asset+'.json')).exists():done.append(key);print('SKIP',key);continue
  tmp=Path(tempfile.mkdtemp())
  try:
   trace={};W,H,cells=render_cell_cycles(dep,str(data),trace);col=collision(data/(dep['bma']+'.bma'));marks,sp=entities(dep['bpl']);asset=dep['bpl'].lower();root=Path(a.out);gp=root/'Grounds'/f'{asset}.rsground';info=write_ground_cells(gp,asset,asset+'_Base',W,H,cells,col,marks,sp)
   ...
   outputs={'ground':gp,'tile':root/'Tiles'/(asset+'_Base.tile')};write_manifest(key,inputs,outputs,{**trace,**info},root/'Manifests'/(asset+'.json'));done.append(key);print('OK',key,info)
